...
import { getCategories, getProducts, getCustomers, getOrders } from "@/apis"
...
export const Dashbaord = () => {
  ...
  const loadStats = async () => {
    try {
      ...
      setStats({
        products: productsRes.totalElements || productsRes.data?.length || 0,
        categories: categoriesRes.totalElements || categoriesRes.data?.length || 0,
        customers: customersRes.totalElements || customersRes.data?.length || 0,
        orders: ordersRes.totalElements || ordersRes.data?.length || 0,
      })
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-border rounded-lg p-6">
            <h2 className="text-xl font-semibold text-foreground mb-4">Información del Sistema</h2>
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">Accede a las secciones principales desde el menú lateral</p>
            </div>
          </div>
        </div>
      </div>
